...
class Warehouse(TenantAwareModel):
    """Almacén o bodega - Aislado por tenant"""
...
class Stock(TenantAwareModel):
    """Stock de productos en almacenes - Aislado por tenant"""
...
class StockMovement(TenantAwareModel):
    """Movimientos de inventario - Aislado por tenant"""
    MOVEMENT_TYPES = [
        ('in', 'Entrada'),
        ('out', 'Salida'),
...
class StockAlert(TenantAwareModel):
    """Alertas de stock - Aislado por tenant"""
    ALERT_TYPES = [
...
class InventoryAdjustment(TenantAwareModel):
    """Ajustes de inventario - Aislado por tenant"""
    warehouse = models.ForeignKey(Warehouse, on_delete=models.CASCADE, verbose_name="Almacén")
...
class InventoryAdjustmentItem(TenantAwareModel):
    """Elementos de ajuste de inventario - Aislado por tenant"""
